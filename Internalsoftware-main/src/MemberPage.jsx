...
export default function MemberPage() {
  ...
  const onePlusOneSchemes = [
   ...
   const handleCustomerPhotoChange = async (e) => {
    ...
    if (originalSizeInMB > 10) {
      toast.error("Image is too large (>10MB). Please select a smaller image.");
      return;
    }
    ...
    try {
      ...
      toast.success(`Photo loaded and compressed to ${compressedSizeInMB.toFixed(2)}MB ✓`);
    } catch (err) {
      console.error('Error processing photo:', err);
      setUploadingPhoto(false);
      toast.error('Photo processing failed. Please try a different image.');
    }
  };
  ...
  const handleExcelUpload = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        toast.error('Excel upload failed. Please check the file format and try again.');
      }
    };
  ...
  const handleUpdateCustomer = async () => {
    ...
    if (!customerInput.name.trim() || !customerInput.mobile.trim()) { toast.warn("Please fill in Name and Mobile fields"); return; }
  ...
  const handleSaveCustomer = async () => {
    if (!selectedVillageid) { toast.warn("Please select a village first"); return; }
    if (uploadingPhoto) { toast.warn('Please wait until photo upload completes'); return; }
    if (!customerInput.name.trim() || !customerInput.mobile.trim()) { toast.warn("Please fill in Name and Mobile fields"); return; }
